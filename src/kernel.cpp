...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CLegacysJDCStake(spend));
        ...
        if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start() ||
            ...
            return error("%s : sJDC stake block: height %d outside range", __func__, (nPreviousBlockHeight+1));
        CLegacysJDCStake* sJDC = dynamic_cast<CLegacysJDCStake*>(stake.get());
        if (!sJDC) return error("%s : dynamic_cast of stake ptr failed", __func__);
        // The checkpoint needs to be from 200 blocks ago
        const int cpHeight = nPreviousBlockHeight - Params().Zerocoin_RequiredStakeDepth();
        const libzerocoin::CoinDenomination denom = libzerocoin::AmountToZerocoinDenomination(sJDC->GetValue());
        if (ParseAccChecksum(chainActive[cpHeight]->nAccumulatorCheckpoint, denom) != sJDC->GetChecksum())
        ...
        CJDCStake* kkcInput = new CJDCStake();
