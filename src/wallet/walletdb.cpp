...
bool CWalletDB::WritesJDCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::ErasesJDCSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WritesJDCSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::ErasesJDCSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadsJDCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadsJDCSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WritesJDCCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadsJDCCount(uint32_t& nCount)
{
