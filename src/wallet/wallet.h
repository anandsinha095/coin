...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4,           // ONLY_NONDENOMINATED and not 10000 JDC at the same time
    ONLY_10000 = 5,                                 // find masternode outputs including locked ones (use with caution)
    STAKEABLE_COINS = 6                             // UTXO's that are valid for staking
};

// Possible states for sJDC send
enum ZerocoinSpendStatus {
    sJDC_SPEND_OKAY = 0,                            // No error
    sJDC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    sJDC_WALLET_LOCKED = 2,                         // Wallet was locked
    sJDC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    sJDC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    sJDC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    sJDC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    sJDC_TRX_CREATE = 7,                            // Everything related to create the transaction
    sJDC_TRX_CHANGE = 8,                            // Everything related to transaction change
    sJDC_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    sJDC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    sJDC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    sJDC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    sJDC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    sJDC_SPENT_USED_sJDC = 14,                      // Coin has already been spend
    sJDC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    sJDC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 JDC output and keys which can be used for the Masternode
    ...
    bool CreatesJDCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // sJDC wallet
    CsJDCWallet* zwalletMain;
    std::unique_ptr<CsJDCTracker> skkcTracker;
    void setZWallet(CsJDCWallet* zwallet);
    CsJDCWallet* getZWallet();
    ...
    // sJDC reset
    boost::signals2::signal<void()> NotifysJDCReset;
};
