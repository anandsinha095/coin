...
public:
    ...
    bool WritesJDCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadsJDCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadsJDCSeed_deprecated(uint256& seed);
    bool ErasesJDCSeed();
    bool ErasesJDCSeed_deprecated();

    bool WritesJDCCount(const uint32_t& nCount);
    bool ReadsJDCCount(uint32_t& nCount);
