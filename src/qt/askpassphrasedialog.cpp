...
#include "qt/jdcoin/loadingdialog.h"
...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                tr("Warning: If you encrypt your wallet and lose your passphrase, you will <b>LOSE ALL OF YOUR JDC</b>!") + "<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                JdcoinGUI* window = static_cast<JdcoinGUI*>(parentWidget());
...
{
    JdcoinGUI* gui = static_cast<JdcoinGUI*>(parentWidget());
...
{
    hide();
    static_cast<JdcoinGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("Jdcoin will close now to finish the encryption process. "
               "Remember that encrypting your wallet cannot fully protect "
               "your JDCs from being stolen by malware infecting your computer.") +
            "<br><br><b>" +
