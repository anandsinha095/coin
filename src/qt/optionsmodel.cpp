...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::JDC);
    ...
    if (settings.contains("nAnonymizeJdcoinAmount") || reset)
        SoftSetArg("-anonymizejdcoinamount", settings.value("nAnonymizeJdcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in jdcoin.cpp during shotdown to also remove the window positions
...
/** Updates current unit in memory, settings and Q_EMITs displayUnitChanged(newUnit) signal */
