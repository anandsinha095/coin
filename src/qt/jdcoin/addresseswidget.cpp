...
#include "qt/jdcoin/forms/ui_addresseswidget.h"
#include "qt/jdcoin/addresslabelrow.h"
#include "qt/jdcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(JdcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter Jdcoin address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress kkcAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(kkcAdd.Get(), label.toUtf8().constData(),
                kkcAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
