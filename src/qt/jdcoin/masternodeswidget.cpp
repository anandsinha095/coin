...
#include "qt/jdcoin/forms/ui_masternodeswidget.h"
#include "qt/jdcoin/qtutils.h"
#include "qt/jdcoin/mnrow.h"
#include "qt/jdcoin/mninfodialog.h"

#include "qt/jdcoin/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(JdcoinGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= (COIN * 10000)){
            inform(tr("Not enough balance to create a masternode, 10,000 JDC required."));
            return;
        }
        showHideOp(true);
