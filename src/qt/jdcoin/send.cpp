...
#include "qt/jdcoin/send.h"
#include "qt/jdcoin/forms/ui_send.h"
#include "qt/jdcoin/addnewcontactdialog.h"
#include "qt/jdcoin/qtutils.h"
#include "qt/jdcoin/sendchangeaddressdialog.h"
#include "qt/jdcoin/optionbutton.h"
#include "qt/jdcoin/sendconfirmdialog.h"
#include "qt/jdcoin/myaddressrow.h"
#include "qt/jdcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(JdcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("JDC");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("sJDC");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (JDC) or private coins (sJDC)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Jdcoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 JDC");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onJDCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onJDCSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool issJDC = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, issJDC));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (issJDC ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendJDC = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendJDC ? AskPassphraseDialog::Context::Send_JDC : AskPassphraseDialog::Context::Send_sJDC, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendJDC) ? send(recipients) : sendsJDC(recipients)) {
...
bool SendWidget::sendsJDC(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("sJDC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from sJDC selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!sJDCControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = sJDCControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendsJDC(
        ...
        inform(tr("sJDC transaction sent!"));
        sJDCControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == sJDC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 sJDC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one sJDC transaction
        ...
        Q_EMIT message("sJDC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isJDC){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any JDC to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            sJDCControlDialog *sJDCControl = new sJDCControlDialog(this);
            sJDCControl->setModel(walletModel);
            sJDCControl->exec();
            ui->btnCoinControl->setActive(!sJDCControlDialog::setSelectedMints.empty());
            sJDCControl->deleteLater();
        } else {
            inform(tr("You don't have any sJDC in your balance to select."));
        }
    }
}
...
void SendWidget::onJDCSelected(bool _isJDC){
    isJDC = _isJDC;
    setCssProperty(coinIcon, _isJDC ? "coin-icon-kkc" : "coin-icon-skkc");
    refreshView();
