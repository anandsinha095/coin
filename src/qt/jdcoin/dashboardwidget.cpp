...
#include "qt/jdcoin/dashboardwidget.h"
#include "qt/jdcoin/forms/ui_dashboardwidget.h"
#include "qt/jdcoin/sendconfirmdialog.h"
#include "qt/jdcoin/txrow.h"
...
DashboardWidget::DashboardWidget(JdcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of JDC and sJDC staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareJDC, "square-chart-kkc");
    setCssProperty(ui->labelSquaresJDC, "square-chart-skkc");
    setCssProperty(ui->labelJDC, "text-chart-kkc");
    setCssProperty(ui->labelsJDC, "text-chart-skkc");
    ...
    ui->labelAmountsJDC->setText("0 sJDC");
    ui->labelAmountJDC->setText("0 JDC");
    setCssProperty(ui->labelAmountJDC, "text-stake-kkc-disable");
    setCssProperty(ui->labelAmountsJDC, "text-stake-skkc-disable");
    ...
    ui->btnHowTo->setText(tr("How to get JDC or sJDC"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("JDC")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isJDC = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakesJDC;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair JDC, sJDC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            chartData->totalJDC += pair.first;
            chartData->totalsJDC += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesJDC.append(kkc);
        chartData->valuessJDC.append(skkc);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("JDC");
    set1 = new QBarSet("sJDC");
    ...
    set0->append(chartData->valuesJDC);
    set1->append(chartData->valuessJDC);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalJDC > 0 || chartData->totalsJDC > 0) {
        setCssProperty(ui->labelAmountJDC, "text-stake-kkc");
        setCssProperty(ui->labelAmountsJDC, "text-stake-skkc");
    } else {
        setCssProperty(ui->labelAmountJDC, "text-stake-kkc-disable");
        setCssProperty(ui->labelAmountsJDC, "text-stake-skkc-disable");
    }
    forceUpdateStyle({ui->labelAmountJDC, ui->labelAmountsJDC});
    ui->labelAmountJDC->setText(GUIUtil::formatBalance(chartData->totalJDC, nDisplayUnit));
    ui->labelAmountsJDC->setText(GUIUtil::formatBalance(chartData->totalsJDC, nDisplayUnit, true));
