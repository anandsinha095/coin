...
#include "qt/jdcoin/topbar.h"
#include "qt/jdcoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/jdcoin/lockunlock.h"
#include "qt/jdcoin/qtutils.h"
#include "qt/jdcoin/receivedialog.h"
...
TopBar::TopBar(JdcoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablesJDC, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingsJDC, ui->labelTitleImmaturesJDC};
    ...
    setCssProperty({ui->labelAmountTopJDC, ui->labelAmountTopsJDC}, "amount-small-topbar");
    setCssProperty({ui->labelAmountJDC, ui->labelAvailablesJDC}, "amount-topbar");
    setCssProperty({ui->labelPendingJDC, ui->labelPendingsJDC, ui->labelImmatureJDC, ui->labelImmaturesJDC}, "amount-small-topbar");
...
void TopBar::loadWalletModel() {
    ...
    // update the display unit, to not use the default ("Jdcoin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // Set
    QString totalJDC = GUIUtil::formatBalance(kkcAvailableBalance, nDisplayUnit);
    QString totalsJDC = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // JDC
    // Top
    ui->labelAmountTopJDC->setText(totalJDC);
    // Expanded
    ui->labelAmountJDC->setText(totalJDC);
    ui->labelPendingJDC->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureJDC->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for sJDC balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of sJDC label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopsJDC->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopsJDC->setText(totalsJDC);
    // Expanded
    ui->labelAvailablesJDC->setText(totalsJDC);
    ui->labelPendingsJDC->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturesJDC->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
