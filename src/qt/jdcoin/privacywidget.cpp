...
#include "qt/jdcoin/privacywidget.h"
#include "qt/jdcoin/forms/ui_privacywidget.h"
#include "qt/jdcoin/qtutils.h"
#include "guiutil.h"
#include "qt/jdcoin/denomgenerationdialog.h"
#include "qt/jdcoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(JdcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting sJDC anonymizes your JDC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new sJDC or convert back to JDC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 JDC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 sJDC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 sJDC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 sJDC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 sJDC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 sJDC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 sJDC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 sJDC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 sJDC");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last sJDC Movements"));
    ...
    // Only Convert to JDC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalsJDC->setTitleClassAndText("btn-title-grey", "Total 0 sJDC");
    ui->btnTotalsJDC->setSubTitleClassAndText("text-subtitle", "Show denominations of sJDC owned.");
    ui->btnTotalsJDC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select JDC outputs to mint into sJDC.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent sJDC");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalsJDC, SIGNAL(clicked()), this, SLOT(onTotalsJDCClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint sJDC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of JDC to mint into sJDC"));
    }else{
        btnText = tr("Convert back to JDC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of sJDC to convert back into JDC"));
    }
...
void PrivacyWidget::onTotalsJDCClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalsJDC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalsJDC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("sJDC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_sJDC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 sJDC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("sJDC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBacksJDC(
            value,
            selectedMints,
            receipt
    )){
        ...
        // Spend succeed
        inform(tr("sJDC converted back to JDC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " sJDC </b>";
    ...
    ui->btnTotalsJDC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
