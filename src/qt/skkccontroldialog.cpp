...
std::set<std::string> sJDCControlDialog::setSelectedMints;
std::set<CMintMeta> sJDCControlDialog::setMints;

bool CsJDCControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == sJDCControlDialog::COLUMN_DENOMINATION || column == sJDCControlDialog::COLUMN_VERSION || column == sJDCControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


sJDCControlDialog::sJDCControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::sJDCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select sJDC Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelsJDC->setProperty("cssClass", "text-main-purple");
    ui->labelsJDC_int->setProperty("cssClass", "text-main-purple");
...
sJDCControlDialog::~sJDCControlDialog()
{
    delete ui;
}

void sJDCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void sJDCControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CsJDCControlWidgetItem* itemDenom(new CsJDCControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CsJDCControlWidgetItem *itemMint = new CsJDCControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend sJDC.";
            else if (!mint.isSeedCorrect)
                strReason = "The sJDC seed used to mint this sJDC is not the same as currently hold in the wallet";
            else
...
void sJDCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void sJDCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelsJDC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setsJDCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> sJDCControlDialog::GetSelectedMints()
{
...
void sJDCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
