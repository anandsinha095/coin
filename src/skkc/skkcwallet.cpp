...
CsJDCWallet::CsJDCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing skkc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadsJDCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasesJDCSeed_deprecated()) {
                    LogPrintf("%s: Updated sJDC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old skkc seed\n", __func__);
                }
            }
        }
    }
...
bool CsJDCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritesJDCCount(nCountLastUsed);
    else if (!walletdb.ReadsJDCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CsJDCWallet::Lock()
{
    seedMaster = 0;
}

void CsJDCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CsJDCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTosJDC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CsJDCWallet::LoadMintPoolFromDB()
{
...
void CsJDCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CsJDCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CsJDCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->skkcTracker->HasPubcoinHash(pMint.first)) {
...
bool CsJDCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTosJDC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to skkcTracker which also adds to database
    pwalletMain->skkcTracker->Add(dMint, true);
...
void CsJDCWallet::SeedTosJDC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CsJDCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CsJDCWallet::GenerateDeterministicsJDC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CsJDCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CsJDCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CsJDCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
