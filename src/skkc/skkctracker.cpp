...
CsJDCTracker::CsJDCTracker(std::string strWalletFile)
{
...
CsJDCTracker::~CsJDCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CsJDCTracker::Init()
{
...
bool CsJDCTracker::Archive(CMintMeta& meta)
{
...
bool CsJDCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CsJDCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CsJDCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CsJDCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CsJDCTracker::GetSerialHashes()
{
...
CAmount CsJDCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CsJDCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CsJDCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CsJDCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CsJDCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CsJDCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CsJDCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CsJDCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CsJDCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CsJDCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CsJDCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CsJDCWallet* sJDCWallet)
{
    bool issJDCWalletInitialized = (NULL != sJDCWallet);
    CMintMeta meta;
    ...
    if (! issJDCWalletInitialized)
        sJDCWallet = new CsJDCWallet(strWalletFile);
    meta.isSeedCorrect = sJDCWallet->CheckSeed(dMint);
    if (! issJDCWalletInitialized)
        delete sJDCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CsJDCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CsJDCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CsJDCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CsJDCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CsJDCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CsJDCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CsJDCWallet* sJDCWallet = new CsJDCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, sJDCWallet);
        }
        delete sJDCWallet;
