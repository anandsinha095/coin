...
bool CJDCStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CJDCStake::GetTxFrom(CTransaction& tx) const
{
    tx = txFrom;
    return true;
}

bool CJDCStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CJDCStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CJDCStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CJDCStake::GetUniqueness() const
{
    //The unique identifier for a JDC stake is the outpoint
...
CBlockIndex* CJDCStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
