...
#if defined(NDEBUG)
#error "Jdcoin cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, int nHeight, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : sJDC spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the sJDC is properly signed
    if (nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 sJDC spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend sJDC without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(fUseV1Params)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(nHeight))
            return error("%s : sJDC spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!sJDCModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!sJDCModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : sJDC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                PublicCoinSpend publicSpend(params);
                if (!sJDCModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                    return false;
                }
            ...
            // Reject legacy sJDC mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include sJDC mint output in tx %s",
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool issJDCStake)
{
    int64_t ret = 0;
    ...
    if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 1) {
        ...
        //When sJDC is staked, masternode only gets 2 JDC
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!sJDCModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Jdcoin
...
void RecalculatesJDCMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    const int chainHeight = chainActive.Height();
    uiInterface.ShowProgress(_("Recalculating minted sJDC..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted sJDC..."), percent);
        }
...
void RecalculatesJDCSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    const int chainHeight = chainActive.Height();
    uiInterface.ShowProgress(_("Recalculating spent sJDC..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainHeight - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent sJDC..."), percent);
        }
        ...
        //Remove spends from sJDC supply
...
bool RecalculateJDCSupply(int nHeightStart)
{
    const int chainHeight = chainActive.Height();
    ...
    uiInterface.ShowProgress(_("Recalculating JDC supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating JDC supply..."), percent);
        }
...
bool UpdatesJDCSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the sJDC supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculatesJDCMinted();
        RecalculatesJDCSpent();
        RecalculateJDCSupply(Params().Zerocoin_StartHeight());
    }

    //Track sJDC money supply in the block index
    if (!UpdatesJDCSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new sJDC supply for block=%s height=%d", __func__,
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 JDC.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent sJDC spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of sJDC serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Jdcoin Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().JdcoinBadBlockTime()) && (block.nBits == (uint32_t) Params().JdcoinBadBlocknBits())) {
            // accept Jdcoin block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        std::vector<CTxIn> sJDCInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                sJDCInputs.push_back(stakeIn);
            }else{
                kkcInputs.push_back(stakeIn);
            }
        }
        const bool hasJDCInputs = !kkcInputs.empty();
        const bool hassJDCInputs = !sJDCInputs.empty();
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hassJDCInputs && in.IsZerocoinSpend())
            ...
            // Now that this loop if completed. Check if we have sJDC inputs.
            if(hassJDCInputs) {
                for (const CTxIn& sJDCInput : sJDCInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(sJDCInput);
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Jdcoin: We use certain sporks during IBD, so check to see if they are
