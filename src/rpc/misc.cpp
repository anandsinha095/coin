...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total jdcoin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"sJDCsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 sJDC denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 sJDC denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 sJDC denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 sJDC denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 sJDC denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 sJDC denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 sJDC denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 sJDC denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all sJDC denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in jdcoin/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in jdcoin/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        skkcObj.push_back(Pair(std::to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    skkcObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("sJDCsupply", skkcObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"jdcoinaddress\"\n"
            "\nReturn information about the given jdcoin address.\n"

            "\nArguments:\n"
            "1. \"jdcoinaddress\"     (string, required) The jdcoin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"jdcoinaddress\",    (string) The jdcoin address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for Jdcoin cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Jdcoin address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are jdcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) jdcoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"jdcoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"jdcoinaddress\"  (string, required) The jdcoin address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,    (boolean) if staking is enabled/disabled in jdcoin.conf\n"
