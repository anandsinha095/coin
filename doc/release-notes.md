...
Jdcoin Core version *version* is now available from:  <https://github.com/jdcoin-project/jdcoin/releases>
...
You can read more details about this extensive work in ([PR #954](https://github.com/Jdcoin-Project/Jdcoin/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/Jdcoin-Project/Jdcoin/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [Jdcoin Wiki](https://github.com/Jdcoin-Project/Jdcoin/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/Jdcoin-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/Jdcoin-Project/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/Jdcoin-Project/Jdcoin/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.

The default split threshold remains at 2000 JDC, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
### V1 sJDC Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 sJDC have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 sJDC with public spends version 4 ([PR #936](https://github.com/Jdcoin-Project/Jdcoin/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 sJDC), version 4 spends will also be public. A full technical writeup is available on the [Jdcoin Wiki](https://github.com/Jdcoin-Project/Jdcoin/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/Jdcoin-Project/Jdcoin/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/Jdcoin-Project/Jdcoin/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/Jdcoin-Project/Jdcoin/pull/975)) and `SPORK_18` ([#PR995](https://github.com/Jdcoin-Project/Jdcoin/pull/995)).<br>
...
`SPORK_18` (off by default) is used to switch between Version 3 and [Version 4 Public Spends](#v1-skkc-spending-public-spends-version-4). When this spork is active, only version 4 spends are accepted by the network. When it's not, only version 3 spends are accepted.
...
- `delegated_balance` - JDC balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - JDC balance held in cold staking addresses.
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~jdcoin) repository, we are now offering a [Snap package](https://snapcraft.io/jdcoin-core) as quick way to install and update a Jdcoin wallet.
